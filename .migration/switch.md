...
`components/ui/switch.tsx` — untouched by this project, so the CLI handled it
after the style flip. `Thumb` is unchanged in name and role.
...
thumb travels and that Space toggles it.
